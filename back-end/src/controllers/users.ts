...
const verifyEmail: RequestHandler = async (req, res) => {
    try {
        const user: UserType = await User.findOne({ token: req.params.emailTokenId });
        ...
        res.status(500).json({ message: 'Something went wrong with the account authentication.', error });
    }
}
...
const signUpUser: RequestHandler = async (req, res) => {
    ...
    try {
        ...
        await newUser.save();
        const msg = createMsg(user, req.headers.host, form.token)
        try {
            await sgMail.send(msg);
